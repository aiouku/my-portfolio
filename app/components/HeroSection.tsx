...
const techStack = ["Unity", "Next.js", "p5.js", "TypeScript", "React"];

export default function HeroSection() {
    const typedText = useTypingEffect(
        "Student Developer • Unity / Web / p5.js",
        40,
        800
    );
    ...
    return (
        ...
        >
            <div className="relative z-10 mx-auto max-w-5xl text-center">
                ...
                {/* Tech Stack */}
                <div
                    className={`transition-all duration-700 delay-600 ${isVisible ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0"
                        }`}
                >
                    <p className="mb-4 text-sm text-zinc-500">Tech I work with</p>
                    <div className="flex flex-wrap items-center justify-center gap-3">
                        {techStack.map((tech) => (
                            <span
                                key={tech}
                                className="rounded-full border border-zinc-700/50 bg-zinc-800/50 px-4 py-2 text-sm text-zinc-300 transition-all duration-300 hover:border-cyan-500/30 hover:bg-cyan-500/10 hover:text-cyan-300"
                            >
                                {tech}
                            </span>
                        ))}
